...
3. Replace externs in the C file.
   ...
   - Use explicit zero initialization so definitions are emitted in source order instead of as reorderable common symbols, e.g. `s32 foo __attribute__((section(".bss"))) = 0;` or `u8 buffer[0x10] __attribute__((section(".bss"))) = { 0 };`.
