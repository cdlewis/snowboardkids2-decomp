...
void initLevelSelectState(void) {
    ...
    setViewportTransformById(state->cameraNode.viewportId, transformPtr);
    ...
    if (gGameSessionContext->customLapEnabled != 0 && gGameSessionContext->gameMode == 1) {
...
s32 buildUnlockedLevelList(u8 *levelIdList) {
    ...
    count = 0;
    maxSlots = gGameSessionContext->gameMode ? 15 : 12;
