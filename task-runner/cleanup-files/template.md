...
Your job is to carefully analyse the functions and variable names in $INPUT, think about what the original developers intended, understand the purpose of the functions and variables, and rename them to better reflect this purpose.

Specifically you should:

- Rename functions to have a more suitable function name if the current name does not adequately reflect its purpose;
...
- Rename struct fields to have a more suitable name, if the fields purpose is clear from the function / other functions / surrounding context. This is particularly true for placeholder fields (those starting with `unk`). For example `arg0->unk0` might be better named `arg0->assetIndex`.
- Update local variables within functions if they have overly generic names or names that do not reflect their purpose;
- Clean up functions to use better spacing, proper struct/array accesses/types, etc as long as doing so does not break the 100% match we require all functions to maintain.
