...
from tools.course_position_common import unpack_gold_coin_positions, write_position_manifest
from tools.sno import decompress_sno_with_consumed


class N64SegGold_coin_positions(CommonSegment):
    @staticmethod
    def is_data() -> bool:
        return True
    ...
    def out_path(self) -> Path:
        return options.opts.asset_path / "course_positions" / "gold_coins" / f"{self.name}.yaml"
    ...
    def split(self, rom_bytes: bytes):
        ...
        if decompressed_size is None:
            log.error(f"gold coin positions segment {self.name} needs decompressed_size")
        ...
        manifest = unpack_gold_coin_positions(self.name, data, decompressed_size, compressed[consumed:])
